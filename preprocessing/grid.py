...
Takes as input two .nii files : vessel_mask and mask file.
Outputs one .nii file with CAPTCHA square where vessels are present : _grid
"""
import argparse
import sys
import os
...
def main(args):
    ...
    input_list = [item for item in unfiltered_filelist if re.search('vessel_mask.nii', item)]
    mask_list = [item for item in unfiltered_filelist if re.search('\\\\mask', item)]
    ...
    for i, j in enumerate(input_list):
        ...
        for l in z_slices:
            ...
            for m in range(num_of_x_patches):
                for n in range(num_of_y_patches):
                    ...
                    #set the squares to 2 when a patch is labeled as containing a vessel
                    label_patch = img_mat[patch_start_x: patch_end_x, patch_start_y: patch_end_y, l]
                    if (1 in label_patch):
        ...
        # SAVE AS NIFTI
        create_and_save_nifti(prob_mat, grid_filepath + j.split(os.sep)[-1].split('_')[0] + '_grid_NEW.nii')
    print('DONE')
