...
def main(args):
    ...
    input_list = [item for item in unfiltered_filelist if re.search('angio.nii', item)]
    ## mask_list is the mask.nii files, the files with the brain masked out
    mask_list = [item for item in unfiltered_filelist if re.search('\\\\mask.nii', item)]
    # label_list is the vessel_mask files, the files that mask out the vessels
    label_list = [item for item in unfiltered_filelist if re.search('vessel_mask.nii', item)]
    ...
    for i,j in enumerate(input_list):
        ...
        helper.create_and_save_nifti(img_mat, target_dir + j.split(os.sep)[-1].split('_')[0] + '_img_NEW.nii')
        helper.create_and_save_nifti(mask_mat, target_dir + j.split(os.sep)[-1].split('_')[0] + '_mask_NEW.nii')
        helper.create_and_save_nifti(label_mat, target_dir + j.split(os.sep)[-1].split('_')[0] + '_label_NEW.nii')
